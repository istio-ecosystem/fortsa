#!/usr/bin/env bash
# End-to-end test: Istio revision tags, fortsa MWC watch triggers restart
# Installs two Istio revisions (stable/canary), updates stable tag to point to canary,
# verifies fortsa restarts the hello-stable deployment.
set -euo pipefail
...
NAMESPACE_STABLE="hello-stable"
NAMESPACE_CANARY="hello-canary"
...
log "Creating namespaces $NAMESPACE_STABLE and $NAMESPACE_CANARY"
kubectl create namespace "$NAMESPACE_STABLE" 2>/dev/null || true
kubectl create namespace "$NAMESPACE_CANARY" 2>/dev/null || true
kubectl label namespace "$NAMESPACE_STABLE" istio.io/rev=stable --overwrite
kubectl label namespace "$NAMESPACE_CANARY" istio.io/rev=canary --overwrite

log "Deploying hello-world app to both namespaces"
kubectl apply -f "$SCRIPT_DIR/hello-world.yaml" -n "$NAMESPACE_STABLE"
kubectl apply -f "$SCRIPT_DIR/hello-world.yaml" -n "$NAMESPACE_CANARY"

log "Waiting for hello-world deployments and sidecar injection..."
kubectl rollout status deployment/helloworld -n "$NAMESPACE_STABLE" --timeout=120s
kubectl rollout status deployment/helloworld -n "$NAMESPACE_CANARY" --timeout=120s

for ns in "$NAMESPACE_STABLE" "$NAMESPACE_CANARY"; do
  for i in $(seq 1 30); do
    if kubectl get pods -n "$ns" -l app=helloworld -o jsonpath='{.items[0].spec.containers[*].name}' 2>/dev/null | grep -q istio-proxy; then
      break
    fi
    sleep 2
  done
  if ! kubectl get pods -n "$ns" -l app=helloworld -o jsonpath='{.items[0].spec.containers[*].name}' 2>/dev/null | grep -q istio-proxy; then
    echo "Error: hello-world pod in $ns did not get istio-proxy sidecar"
    kubectl get pods -n "$ns" -o wide
    kubectl describe pod -n "$ns" -l app=helloworld
    exit 1
  fi
  log "Hello-world pod in $ns has istio-proxy sidecar"
done
...
# Record initial stable pod before tag change
INITIAL_POD=$(kubectl get pods -n "$NAMESPACE_STABLE" -l app=helloworld -o jsonpath='{.items[0].metadata.name}')
log "Initial stable pod: $INITIAL_POD"

log "Updating stable tag to point to ${REVISION_NEW} (same as canary)..."
"$ISTIOCTL_NEW" tag set stable --revision "$REVISION_NEW" --overwrite

wait_delay=2 # seconds
wait_time=$((60 * 5)) # 5 minutes
wait_loops=$((wait_time / wait_delay))
log "Waiting for fortsa to annotate hello-stable deployment and trigger restart (up to $wait_time seconds)..."
SUCCESS=0
# shellcheck disable=SC2034
for i in $(seq 1 $wait_loops); do
  if kubectl get deployment helloworld -n "$NAMESPACE_STABLE" -o jsonpath='{.spec.template.metadata.annotations.fortsa\.scaffidi\.net/restartedAt}' 2>/dev/null | grep -q .; then
    log "Found restartedAt annotation on helloworld deployment in $NAMESPACE_STABLE"
    CURRENT_POD=$(kubectl get pods -n "$NAMESPACE_STABLE" -l app=helloworld -o jsonpath='{.items[0].metadata.name}' 2>/dev/null || true)
...
if [[ $SUCCESS -ne 1 ]]; then
  echo "Error: fortsa did not restart the helloworld deployment in $NAMESPACE_STABLE within $wait_time seconds"
  echo "Deployment annotations:"
  kubectl get deployment helloworld -n "$NAMESPACE_STABLE" -o yaml | grep -A 20 "annotations:"
  echo "Fortsa logs:"
...
NEW_POD=$(kubectl get pods -n "$NAMESPACE_STABLE" -l app=helloworld -o jsonpath='{.items[0].metadata.name}')
PROXY_IMAGE=$(kubectl get pod "$NEW_POD" -n "$NAMESPACE_STABLE" -o jsonpath='{.spec.containers[?(@.name=="istio-proxy")].image}')
...
if [[ "$PROXY_IMAGE" != *"$ISTIO_NEW_VERSION"* ]]; then
  ...
  kubectl rollout status deployment/helloworld -n "$NAMESPACE_STABLE" --timeout=60s
fi

log "E2E test passed: fortsa successfully restarted helloworld deployment in $NAMESPACE_STABLE after stable tag update"
